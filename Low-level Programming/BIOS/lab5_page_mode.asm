...
;PDE		equ 5000h
;CODE_PG	equ 8F0000h		;новый сегмент кода (пункт c)
;BDA_PG		equ 400000h		;физ адреса для Bios Data Area
;DEVICE_PG	equ 1400000h		;лин. адреса для физических устройств
;--------------------------------UART-----------------------------------
COM1		equ 3F8h
COM2		equ 2F8h
COM3		equ 3E8h
...
NEWGDTR:	dw GDT_SIZE-1		;новая табличка на 8м мегабайте (на самом деле не новая а скопированная старая
		dd 0800000h+(STARTGDT SHL 4)
NEWIDTR:	dw IDT_SIZE-1
		dd 0800000h+(STARTIDT SHL 4)

create_page macro lin, phys, properties:vararg
		_g=1			;глобальная страница
		_ps=1			;размер страницы
		_access=0
		_pcd=0			;бит сброшен -> кэширование страниц разрешено  (ДЛЯ ФИЗ УСТРОЙСТВ ВСЕГДА ВЗВЕДЕН!)
		_pwt=1			;если 1 то write-through кэширование разрешено
		_rw=1			;r/w
		_user=1			
		_p=1			;бит присутствия

mov edi, PDE+(lin SHR 22)*4
mov eax, ((phys AND 0FFFFF000h)+(_g SHL 8)+(_ps SHL 7)+(_access SHL 5)+(_pcd SHL 4)+(_pwt SHL 3)+(_user SHL 2)+(_rw SHL 1)+_p)
stosd
endm

pages_on:
;a) Было включено защищённое страничное преобр: страницы 4M, обычное 32-х битовое страничное преобразование.	....check
;b) Для тех страниц, для которых возможно, использовалось наиболее эффективный режим кэширования.		....check
;c) Для выполняемого кода bios (диапазон физических адресов 0xF0000..0xFFFFF)					....check
	;использовать линейные адреса из диапазона 0x8F0000..0x8FFFFF.
;d) Для диапазона линейных адресов BDA (Bios Data Area) использовать физические адреса, 
	;начинающиеся c адреса 0x400000.
...
;1400000	--->	FE....
		...
		mov edi, PDE				;создаем каталог страниц, размер кактлога 4кб
		mov ecx, 1024				;в 4кб помещается 1024 32битных записи (4байта)
		xor eax, eax	
		rep stosd				;обнуляем элементы каталога
;для 4Мб-страниц таблица страниц никак не используется - все решают PDE
;БАЗУ КОДА CS ПОДВИНУТЬ НА 8Й МЕГАБАЙТ  			(спец задание от Макарова)
		;create_page linear_adr -> phys_adr
		create_page 0, 0			;F0000 или 0
		create_page CODE_PG, 0			;F0000 или 0
		create_page DEVICE_PG, IOAPICB, _pcd=1	;кэшировать адреса физ устройств нельзя
		
		mov eax, PDE				;кладем адрес каталога страниц в %cr3
		mov cr3, eax
		...
		mov eax, cr0				;включаем стр. преобразование
		or	eax, 80000000h
		mov cr0, eax

		lgdt fword ptr NEWGDTR			;копируем таблички обновленные
		lidt fword ptr NEWIDTR

	;передвигаем старый дескриптор кода на 8й МБ
		mov edi, (STARTGDT SHL 4)+DCODE		
		xor eax, eax
		stosd
		mov eax, 0C09B8Fh			;дескр сегмента кода:G=1,B=1,P=1, #43=1 и #44=1, C(code segm)=1, 
							;база сегмента=8F -8й мегабайт, далее 0
		stosd

		db 0EAh					;просто так перейти на след. инструкцию мы не можем, нужен дальний джамп
		dd offset last_page
		dw 8

last_page:
		create_page	0, BDA_PG		;F0000 или 0
		
		mov esp, 0FFCh
		jmp tibet				;стек сбился так что возвращаемся джампом
		push offset tibet
		ret
...
;1200 baud, 7 bit, 1 stop bit, no parity		<---- SERIAL MOUSE (COM)
;------------------------------------------------------------------------
...
GDT:
		dd 0, 0
		descriptor _limit=0Fh, _base=0F0000h, _g=1, _x=1, _d_c=1	;32х разрядный сегмент кода
		descriptor _limit=100h,	_base=400h, _g=0					;16х данные BIOS (BDA)
		descriptor _limit=0FFFFh, _base=90000h, _g=0				;32х разрядный стек
		descriptor _base=1400000h, _g=1, _x=1
		descriptor _base=1600000h, _g=1, _x=1
		descriptor _base=0h, _limit=0Fh, _g=1
GDT_SIZE equ $-GDT
GDTR:	dw GDT_SIZE-1
	dd STARTGDT SHL 4
